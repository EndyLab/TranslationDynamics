...
ipArray=(`aws ec2 describe-instances --filters Name=tag:expts,Values=RepeatCrowder1ResStep_reflectionbounce| grep -i PublicIpAddress  | awk '{ print $2}' | cut -d',' -f1| sed -e 's/"//g'| tr . -`)
i=0
