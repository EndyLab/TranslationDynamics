...
for ip in `aws ec2 describe-instances --filters Name=tag:expts,Values=nano1 --region $REGION | grep -i PublicIpAddress  | awk '{ print $2}' | cut -d',' -f1| sed -e 's/"//g'| tr . -` #for each ip that is running
do		
