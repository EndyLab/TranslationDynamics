...
REGION=us-west-1
ipArray=(`aws --region us-west-1 ec2 describe-instances --filters "Name=availability-zone,Values=us-west-1b" "Name=tag:expts,Values=nocrowder-gr3-lowdiff" | grep -i PublicIpAddress  | awk '{ print $2}' | cut -d',' -f1| sed -e 's/"//g'| tr . -`)
i=0
